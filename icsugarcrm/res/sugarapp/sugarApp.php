...
/* @version $Id: sugarApp.php,v 1.1.1.1 2010/04/15 10:03:40 peimic.comprock Exp $ */
...
class sugarApp {
    ...
    var $globalDescription = '     <h1>Sugar Case Self-Service Portal for Joomla</h1>';
    ...
    // Moved from sugarConfiguration.  The installer should isntantiate the app class and never the config class.

    function installSanityTest() {

        if ( ! $this->checkConfig() ) {

            $descToReturn = '    <h3>You do not appear to have any other components of the Sugar Self-Service Portal for Joomla installed or they are improperly configured</h3>

    <p>If you\'d like to configure your Sugar Case Self-Service Portal, you may go ahead and do so now.  If you choose not

        to configure your Sugar Case Self-Service Portal right now, please be aware that it won\'t work properly,

        if at all, until you do so.</p>';

            $descToReturn .= $this->getConfigForm();

        } else {

            $descToReturn =     '<h2>Sugar Case Self-Service Portal Configuration</h2>

    <p>It appears that you already have a configuration table for the Sugar Self-Service Portal for

        Joomla.  Normally that means this is the second or third component you\'re installing

        as part of the Portal.  If, however, this is because of a previous version/installation

        of the Sugar Self-Service Portal for Joomla that has not been used/installed for some time, there

        might be compatibility problems if this table is different than the table I need.</p>';
