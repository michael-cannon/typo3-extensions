...
class tx_wecservant_pi1 extends tslib_pibase {
	...
	var $curDates;		// passed in (POST) dates
	...
	var $unixYear; // seconds in a year
	var $timeNow; // current time
	var $isYouth				= false; // mark as youth, if such
	var $requiresParent			= false; // if youth and supervision required
	var $isDuplicate			= false; // mark as duplicate, if such
	var	$minSignedUp			= false;
	var $jsCode					= '';	// JavaScript holder
...
//		$GLOBALS['TSFE']->set_no_cache();
		...
		elseif ($this->conf['pid_list'])	// or specify in TypoScript
		...
		$this->db_fields 		 = array('uid','name','datetime','reference_code','description','youth_friendly','supervision_required','ministry_uid','contact_uid','contact_info','location','times_needed','openings','priority','skills','misc_description','qualifications');
		$this->db_minFields 	 = array('uid','name','description');
		$this->db_contactFields  = array('uid','name','email','telephone','address','city','zone','zip');
		$this->db_showFields 	 = array('uid','name','datetime','reference_code','description','youth_friendly','supervision_required','ministry_info','contact_info','contact_phone','contact_email','location','times_needed','openings','priority','skills','misc_description','qualifications');
		$this->db_showFormFields = array('name','email','phone','address','city','state','zip','message','attachment','youth','parents_name','chosen_minopps');
		...
		} elseif ( isset( $this->postvars['email'] ) ) {
			// MLC look up or create user entry
			$this->userID		= $this->get_userId( $this->postvars );
			$this->userName		= $GLOBALS['TSFE']->fe_user->user['username'];
		} else {
			// no user logged in...so clear out vars
			$this->userID		= 0;
			$this->userName		= '';
		}

		if ( $this->userID && $GLOBALS['TSFE']->fe_user->user['date_of_birth'] > 0 ) {
			// unix time of now less ( youth age * unixYear )
			$youthAge	= $this->timeNow - ( $this->conf['youth'] * $this->unixYear );

			// using 17 for youth, 16 for my age and 2000 for current year
			// 1984 >= 1983
			if ( $GLOBALS['TSFE']->fe_user->user['date_of_birth'] >= $youthAge ) {
				$this->isYouth	= true;

				// set requires_supervision if not old enough
				// using 15 for supervision, 14 for my age and 2000 for current year
				// 1986 > 1985
				$supervisionAge	= $this->timeNow - ( $this->conf['supervision_age'] * $this->unixYear );

				if ( $GLOBALS['TSFE']->fe_user->user['date_of_birth'] > $supervisionAge ) {
		...
		$this->config['canFindByDates']		= $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'canFindByDates', 	'sDEF');
		...
		$this->config['hide_view_this']		= $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'hide_view_this', 		's_options');
		$this->config['show_waitlist']		= $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'show_waitlist', 		's_options');
...
//		$this->config['grouptype_id'] 		= $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'grouptype_name', 's_administrator');
        ...
        if ($grpArray = $this->get_groups_by_type($defaultGroupTypeID,"title")) {
			...
			// MLC remove inline counts on for loops
	        $countGrpArray	= count($grpArray);
	        for ($i = 0; $i < $countGrpArray; $i++) {
		...
		if ($this->userID && ($admins = $this->config['administrator_group'])) {
			...
			foreach ($adminList as $thisAdmin) {
				...
				if ($n = strpos($thisAdmin,":")) {
					...
					$countThisMinList	= count($thisMinList);
					for ($j = 0; $j < $countThisMinList; $j++) {
						$found = -1;
						$countMinistryList	= count($this->ministryList);
						for ($k = 0; $k < $countMinistryList; $k++) {
		...
		elseif ($v =  t3lib_div::_GP('findbyministries')) {
			$this->curMinistryUID = $v;
		}
		elseif ($v = $this->conf['showMinistry']) {
			$this->curMinistryUID = $v;
		}
		elseif ($v = htmlspecialchars(t3lib_div::_GP('findbyneeds'))) {
			$this->curPriority = $v;
		}
		elseif (t3lib_div::_GP('findbydates')) {
			$this->curDates = t3lib_div::_GP('findbydates');
			foreach ($this->curDates as $key => $value) {
				if ( ! $value ) {
					switch ( $key ) {
						case 'from':
							$value	= date('n/j/Y');
						...
						case 'to':
						default:
							$value	= date('n/j/Y', strtotime( 'next year' ) );
							break;
					}
				}

				// SECURITY ON incoming vars
				$this->curDates[$key] = htmlspecialchars($value);
			}
		}
		elseif ($v = htmlspecialchars(t3lib_div::_GP('findbylastadded'))) {
			$this->lastAdded = $v;
		}
		elseif (t3lib_div::_GP('findbyskills')) {
		...
		if ($this->isAdministrator || $this->isServantContact)	{
		  	...
		  	elseif ($v = htmlspecialchars(t3lib_div::_GP('adminDel'))) 	{ $delUID = (intval($v) == $v) ? $v : $this->postvars['uid']; $this->admin_delMinOpp($delUID);	}
		  	elseif ($v = htmlspecialchars(t3lib_div::_GP('adminMyList'))) 	{ if ($v != ($this->userID)) {$v=0; $this->action=0;} }
		  	elseif ($v = htmlspecialchars(t3lib_div::_GP('adminEdit'))) 	{ $this->postvars = $this->get_by_minopp($v); }
		  	elseif (t3lib_div::_GP('adminAdd')) 	 { $this->admin_addUpdMinOpp($this->postvars,0);  	}
			elseif (t3lib_div::_GP('adminstats'))   { $this->action = 'admin'; }
		  	elseif (!$this->isAdministrator) 	 { if (!t3lib_div::_GP('admin')) $this->action = 0; } // only allow non-admins to do above actions
		  	elseif (t3lib_div::_GP('admin') == 1) { // if admin=1 then show menu
		  		$this->action = 'admin';
		  	}
		  	else {
		  		$this->action = 0;
			...
			if (($v = htmlspecialchars(t3lib_div::_GP('edit'))) > 1) {
				$this->postvars = $this->get_by_minopp($v);
		...
		if (($showskills = htmlspecialchars(t3lib_div::_GP('showskills_any'))) || ($showskills = htmlspecialchars(t3lib_div::_GP('showskills_all')))) {
			...
			$countMatchSkillList	= count($matchSkillList);
			for ($i = 0; $i < $countMatchSkillList; $i++) {
		...
		//--------------------------------------------------------------
		if ($this->postvars['makecommitment'] || $this->postvars['makewaitlist'] || $this->postvars['showinterest']) {
			$this->action = 'signup';

			// check for supervision requirement when youth_friendly and supervision_required
			$this->isCommitment	= htmlspecialchars($this->postvars['makecommitment']);
			if ($this->isCommitment > 0) 	$this->minSignedUp = $this->isCommitment;

			$this->isWaitlist	= htmlspecialchars($this->postvars['makewaitlist']);
			if ($this->isWaitlist > 0) 	$this->minSignedUp = $this->isWaitlist;

			$this->isInterest	= htmlspecialchars($this->postvars['showinterest']);
			if ($this->isInterest > 0)	$this->minSignedUp = $this->isInterest;
		}

		if ($this->postvars['interestedUID']) {
			$this->isInterested	= htmlspecialchars($this->postvars['interestedUID']);
			if ($this->isInterested > 0) 	$this->minSignedUp = $this->isInterested;
		}

		if ($this->minSignedUp) {
			if ($minOpp = $this->get_by_minopp($this->minSignedUp)) {
				// set if youth, otherwise don't worry for adults
				if ( $minOpp['youth_friendly']
					&& $minOpp['supervision_required']
					&& ( $this->requiresParent || ! $GLOBALS['TSFE']->fe_user->user['date_of_birth'] )
				) {
					$this->config['display_formfields'][]	= 'parents_name';
					$this->config['required_formfields'][]	= 'parents_name';
				}
			}
		}
		...
		if ($printhow = t3lib_div::_GP('printpage')) {
			...
			switch ($printhow) {
				...
				case 4: $this->skillsSearch = $printvar;
						...
						break;
				case 5: $this->curDates = $printvar; break;
			}
		}
...
//			$this->print_saved_minOpp();
		...
		elseif ($baseCSSFile = $this->conf['baseCSSFile']) {
	...
	function display_main() {
		...
		if ((($this->config['canFindByMinistry'] || $this->config['canFindByMinistries']) && $this->curMinistryUID) || ($this->curMinistryUID == -1)) {
			...
			elseif (is_array($this->curMinistryUID	) && (count($this->curMinistryUID) > 1)) {
		...
		// Find By Dates
		if ($this->config['canFindByDates']) {
			$markerArray['###FIND_BY_DATES_HEADER###'] = $this->pi_getLL('find_by_dates','Search By Dates');
			$markerArray['###FIND_BY_DATES###'] = $this->find_by_dates();
			if ($this->curDates) {
				$minArray = $this->get_by_dates($this->curDates);
				$markerArray['###DISPLAY_RESULTS###'] = $this->show_items($minArray);
				$from = $this->pi_getLL('datesFrom', 'From') . ' ' . $this->curDates['from'];
				$to = $this->pi_getLL('datesTo', 'To') . ' ' . $this->curDates['to'];
				$markerArray['###DISPLAY_RESULTS_HEADER###'] = '<h3>'.$this->pi_getLL('find_by_dates_results','Results For Find By Dates:').' '.$from.' '.$to.' ('.count($minArray).' found)</h3>';
			}
		}
		...
		$markerArray['###JAVASCRIPT_CODE###'] = $this->jsCode;
		...
		if (t3lib_div::_GP('printsaved') || t3lib_div::_GP('printpage')) {
			...
			$markerArray['###JAVASCRIPT_CODE###'] = $this->jsCode . $this->add_print_button();
		}
		...
		if ((strlen($markerArray['###DISPLAY_RESULTS###']) > 0) && !t3lib_div::_GP('printsaved') && !t3lib_div::_GP('showsaved')) {
			...
			elseif ($printVars = $this->curPriority) {
				$printHow = 2;
			}
			elseif ($printVars = $this->lastAdded) {
				$printHow = 3;
			}
			elseif ($printVars = $this->skillsSearch) {
				$printHow = 4;
				$param['skillsby'] = $this->skillsSearchBy;
			}
			elseif ($printVars = $this->curDates) {
				$printHow = 5;
			}
...
//			$markerArray['###PRINT_BUTTON###'] = '<div class="button"><a href="'.$this->getAbsoluteURL($this->id,$params).'">'.$this->pi_getLL('print_button','Print Page').'</a></div>';
		...
		if ($thisUID = $this->postvars['single']) {
			$minArray = array($this->get_by_minopp($thisUID));
		...
		if ($this->formErrorText) {
			...
			$subpartMarkerArray['###SHOW_ERRORS###'] = '';
		}
	...
	function show_items($minArray, $showAll = false) {
		...
		if (is_array($this->config['display_oppfields']) && count($this->config['display_oppfields']) > 0) {
			...
			$countFieldArray	= count($fieldArray);
			for ($i = 0; $i < $countFieldArray; $i++)	{
				$fieldFound = false;
		...
		$countMinArray	= count($minArray);
		for ($i = 0; $i < $countMinArray; $i++) {
			$thisItem = $minArray[$i];
			unset($thisMarkerArray);
			$changed = false;
			foreach ($this->db_showFields as $theField)	{
				if ($hideArray[strtoupper($theField)] == 1 || ! isset( $thisItem[$theField]) ) {
					$thisMarkerArray['###SHOW_'.strtoupper($theField).'###'] = 'style="display:none;"';
				}
				if ( isset( $thisItem[$theField] ) ) {
					$thisMarkerArray['###ITEMLABEL_'.strtoupper($theField).'###'] = $this->pi_getLL('itemlabel_'.strtolower($theField));
					switch ( $theField ) {
						case 'openings':
							$thisMarkerArray['###'.strtoupper($theField).'###'] = $thisItem[$theField] ? $thisItem[$theField] : $this->pi_getLL('itemlabel_openings.zero');
							break;

						case 'datetime':
							$thisMarkerArray['###'.strtoupper($theField).'###'] = $thisItem[$theField] ? date($this->conf['datetime'], $thisItem[$theField]) : $this->pi_getLL('itemlabel_datetime.ongoing');
							break;

						case 'youth_friendly':
							$thisMarkerArray['###'.strtoupper($theField).'###'] = $thisItem[$theField] ? $this->pi_getLL('itemlabel_youth_friendly.yes') : $this->pi_getLL('itemlabel_youth_friendly.no');
							break;

						case 'supervision_required':
							$thisMarkerArray['###'.strtoupper($theField).'###'] = $thisItem[$theField] ? $this->pi_getLL('itemlabel_supervision_required.yes') : $this->pi_getLL('itemlabel_supervision_required.no');
							break;
			...
			if ($changed) {
				...
				elseif ($this->conf['displayItemBackColor'])
				...
				if ($showButtons) {
					if ($this->config['can_save_for_later']) {
						...
						$isSaved = "";
						$countSavedMinOpps	= count($this->savedMinOpps);
						for ($k = 0; $k < $countSavedMinOpps; $k++)
					...
					if (($this->config['require_login_to_signup'] == 0) || ($this->userID != 0)) {
						...
						// MLC only show if there's openings > 0 or NULL
						if ($this->showCommitmentForm
							&& ( 0 < $thisItem['openings']
								|| is_null( $thisItem['openings'] ) ) ) {
						...
						} elseif ($this->showCommitmentForm
							&& $this->config['show_waitlist'] ) {
							$extraParams2['tx_wecservant[makewaitlist]'] = $thisItem['uid'];
							$theURL = $this->getAbsoluteURL($this->id,$extraParams2);
						   	$thisMarkerArray['###COMMITMENT_BTN###'] = '<a class="button smallButton" href="'.$theURL.'"><span class="label subscribeIcon">' . $this->pi_getLL('waitlist_btn','Add to Waitlist') . '</span></a>';
						}
					}
	...
	function find_by_needs() {
		...
		// can have up to 9 priority levels...whatever is defined in locallang.xml
		for ($i = 1; $i < 10; $i++) {
	...
	/**
	 * FIND_BY_DATES: Show interface for user to find by dates
	 *
	 * @return	string		content of interface to find
	 */
	function find_by_dates() {
		$selDatessStr = '<form name="finddates" method="post" action="'.$this->getAbsoluteURL($this->id).'" class="selectForm">';
		$selDatessStr .= $this->pi_getLL('datesFrom', 'From') . ' ';
		$selDatessStr .= '<input type="text" name="findbydates[from]" value="'.$this->curDates['from'].'" />';
		$selDatessStr .= ' ';
		$selDatessStr .= $this->pi_getLL('datesTo', 'To') . ' ';
		$selDatessStr .= '<input type="text" name="findbydates[to]" value="'.$this->curDates['to'].'" />';
		$selDatessStr .= '<input type="submit" name="submit" value="Submit" />';
		$selDatessStr .= '</form>';
		...
		if (count($minOppArray)) {
			...
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*',$this->servantSkillsMMTable,$where,"",$orderBy,"");
	 		while ($rowData = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
	 			...
	 			$curMinOpp = 0;
	 			$countMinOppArray	= count($minOppArray);
	 			for ($i = 0; $i < $countMinOppArray; $i++) {
	 				// first, find the minOpp
	 				if ($minOppArray[$i]['uid'] == $whichUID) {
						// then find the skill
		 				$countSkillsList	= count($this->skillsList);
		 				for  ($j = 0; $j < $countSkillsList; $j++) {
	...
	/**
	 * GET BY DATES: Return all ministry opportunities with datetime between given dates
	 *
	 * @param	array		$dates		which dates to match to.
	 * @return	array		$minOppArray	list of all ministry opportunities that match criteria
	 */
	function get_by_dates($dates) {
		$order_by = 'datetime ASC';
		$where .= ' pid IN('.$this->pid_list.')';
		$where .= ' AND (';
		$where .= ' 	datetime = 0';
		$where .= ' 	OR (';
		$where .= ' 		datetime >= '.strtotime($dates['from'].' 00:00:00');
		$where .= ' 		OR datetime <= '.strtotime($dates['to'].' 23:59:59');
		$where .= '		)';
		$where .= '		OR (';
		$where .= '			starttime <= '.strtotime($dates['to'].' 23:59:59');
		$where .= ' 		OR starttime = 0';
		$where .= '		)';
		$where .= '		OR (';
		$where .= '			endtime >= '.strtotime($dates['from'].' 00:00:00');
		$where .= ' 		OR endtime = 0';
		$where .= '		)';
		$where .= ' )';
		// handle languages
	...
	function get_by_skills($matchSkills, $matchAll) {
		...
		foreach ($this->skillsList as $sk) {
			...
			if ($grp && $grp_required) {
				...
				$countMatchSkills	= count($matchSkills) - 1;
				for ($i = $countMatchSkills; $i >= 0; $i--) {
		...
		$where .= ' AND A.starttime<='.$this->timeNow.' AND (A.endtime=0 OR A.endtime>'.$this->timeNow.')';
		// handle languages
		$lang = ($l = $GLOBALS['TSFE']->sys_language_uid) ? $l : '0,-1';
		...
		// now handle if matchAll or matchAny
 		if ($matchAll) { // go through and get rid of any that do not intersect
			...
			if (count($reqSkills)) { // Filter out those not found in reqSkills (if set)
				$newMinOppArray = array();
				$countMinOppArray = count($minOppArray)-1;
				for ($i = $countMinOppArray; $i>=0; $i--) {
	...
	function get_saved_minopp() {
		...
		$this->savedMinOpps = explode(",",$savedList);
		$this->savedMinOpps = array_unique($this->savedMinOpps);
		$minOppArray = $this->get_by_minopp($this->savedMinOpps);
	...
	function display_signup_form() {
		...
		if ($isErrors) {
			...
			$subpartArray['###SHOW_ERRORS###'] = __LINE__;
		...
		if ($this->isCommitment) {
			...
			$markerArray['###HIDDEN_VARS###'] = '<input type="hidden" name="tx_wecservant[signupForm]" value="2"/>
												 <input type="hidden" name="tx_wecservant[makecommitment]" value="'.$this->isCommitment.'"/>';
		} elseif ($this->isWaitlist) {
			$markerArray['###FORM_HEADER###'] = $this->pi_getLL('form_header_waitlist','Add to Waitlist');
			$markerArray['###SIGNUP_TEXT###'] = $this->pi_getLL('form_waitlist_text',	'You are adding yourself to the waitlist to doing the following:');
			$markerArray['###HIDDEN_VARS###'] = '<input type="hidden" name="tx_wecservant[signupForm]" value="3"/>
												 <input type="hidden" name="tx_wecservant[makewaitlist]" value="'.$this->isWaitlist.'"/>';
		} else {
			...
			$markerArray['###HIDDEN_VARS###'] = '<input type="hidden" name="tx_wecservant[signupForm]" value="1"/>
												 <input type="hidden" name="tx_wecservant[showinterest]" value="'.$this->isInterest.'"/>';
		}
		...
		$minListText = "";
		$minListHTML = "";
		if ($this->minSignedUp) { // signed up for one
			if ($minOpp = $this->get_by_minopp($this->minSignedUp)) {
				$minUID = $minOpp['uid'];
				$minListText = $minOpp['name'];
				$minListHTML = '<div class="tx-wecservant-bold">'.$minListText.'</div>';
			}
		} elseif  (($this->isCommitment == -2) || ($this->isInterest == -2)) { // custom servant opp
		...
		foreach ($this->db_showFormFields AS $req_field) {
			$markerArray['###FORM_'.strtoupper($req_field).'###'] = $this->pi_getLL('form_'.strtolower($req_field).'_label',$req_field);
		}
        ...
        if (!$this->postvars['name'] && $GLOBALS['TSFE']->loginUser) {
			...
			// auto select youth if meets requirements
			$markerArray['###VALUE_YOUTH###'] = $this->isYouth ? 'checked="checked"' : '';
    	}
    	...
    	if ($this->postvars && $this->formErrorText) {
			foreach ($this->postvars as $fieldName => $fieldValue) {
				...
				switch ($fieldName) {
					case 'youth':
						if ( $fieldValue) {
							$markerArray['###VALUE_'.strtoupper($fieldName).'###'] = 'checked="checked"';
						} else {
				...
				// for radio buttons...
				// note this fills in all values though because cannot determin if radio button or not
				$fVal = str_replace(' ','_',trim($fieldValue));
				if ( $fVal) {
					$fVal = str_replace('/','-',$fVal); // because regex in substituteArrayMarker needs this fixed
					$markerArray['###VALUE_'.strtoupper($fieldName).'_'.strtoupper($fVal).'###'] = 'checked="checked"';
				}
			}
    	}
	...
	function send_signup_email($formvars)	{
		...
		// MLC data pulled later as well, try to only do once
		$minopp					= $this->get_by_minopp( $minID );

		// MLC is registrant allowed or not
		if ( $this->conf['enableBackgroundCheck'] ) {

			// determine if minopp is youth_friendly
			$isYouthFriendly	= ( isset( $minopp['youth_friendly'] ) && $minopp['youth_friendly'] ) ? true : false;

			$hasBirthDate		= ( $GLOBALS['TSFE']->fe_user->user['date_of_birth'] ) ? true : false;
			$checkedYouth		= ( $formvars['youth'] ) ? true : false;
			// isYouth init confirms date_of_birth with youth age allowance
			// folks without date_of_birth are those we can't confirm so we raise a false positive
			$validCheckedYouth	= ( $checkedYouth && ( $this->isYouth || ! $hasBirthDate ) ) ? true : false;

			$bgCheckDate		= $GLOBALS['TSFE']->fe_user->user['tx_wecservant_bg_check_date'];
			$hasBgCheckDate		= ( $bgCheckDate ) ? true : false;
			$validBgCheckTerm	= $this->unixYear * $this->conf['backgroundCheckValidity'];
			$bgCheckAge			= $this->timeNow - $validBgCheckTerm;
			$isValidBgCheck		= ( $bgCheckDate >= $bgCheckAge ) ? true : false;

			// assume bg passed, then...
			$bgCheckPassed		= true;

			// build up cases in which bg check fails
			// easier logic to follow and handle
			// note: youths are allowed without a valid background check
			// is youth, but opp not kid friendly - disallow
			if ( ( $validCheckedYouth || $this->isYouth ) && ! $isYouthFriendly ) {
				$bgCheckPassed	= false;

				$this->formErrorText = $this->pi_getLL('notKidFriendly','This ministry opportunity is not appropriate for youths. Please volunteer for one that is.');
			} elseif ( ! $this->isYouth && ! $hasBgCheckDate ) {
				// background check required
				$bgCheckPassed	= false;

				$this->formErrorText = $this->pi_getLL('needBackgroundCheck','You must sign up as a volunteer and pass a background check prior to registering for a ministry opportunity.');
				$this->formErrorText .= ' <a href="';
				$this->formErrorText .= $this->getAbsoluteURL( $this->conf['servantRegistrationPID'] );
				$this->formErrorText .= '">';
				$this->formErrorText .= $this->pi_getLL('clickHereToProceed', 'Click here to proceed.');
				$this->formErrorText .= '</a>';
			} elseif ( ! $this->isYouth && ! $isValidBgCheck ) {
				// background checks older than x years need renewal
				$bgCheckPassed	= false;

				$this->formErrorText = $this->pi_getLL('needBackgroundCheckRenewal','You must renew your background check prior to registering for a ministry opportunity.');
				$this->formErrorText .= ' <a href="';
				$this->formErrorText .= $this->getAbsoluteURL( $this->conf['backgroundCheckRenewalPID'] );
				$this->formErrorText .= '">';
				$this->formErrorText .= $this->pi_getLL('clickHereToProceed', 'Click here to proceed.');
				$this->formErrorText .= '</a>';
			}

			// MLC allowing background check failures to go through, but pass the formErrorText to the completed form in hopes that person will click on the background app or renewal and complete such
			if ( ! $this->conf['allowFailedBackgroundChecks']
				&& ! $bgCheckPassed ) {
				return 0;
			}
		}

		$this->isCommitment = ($formvars['signupForm'] == 2);
		...
		if ($minID > 0 && (strpos($minID,',') == false)) {	// add one selected
			array_push($ministryInfo, $minopp);
		} elseif ($minID == -2) { // custom...
			;
		...
		foreach ($ministryInfo as $thisMin) {
			...
			// MLC check that ministry has openings
			if ( ! $this->isWaitlist && '0' == $thisMin['openings'] ) {
				$this->formErrorText = sprintf( $this->pi_getLL('no_openings'), $thisMin['name'] );
				return 0;
			}
		...
		if (($minID == -2) || !count($contactEmailList)) {
			...
			// MLC TODO this is causing basically duplicate entries, but wtih 0 as contact_uid
			// would like to turn off as it's basically useless since nothings done with tracking at this point
			// however, in future, something might be done with tracking, so leaving on
		...
		// MLC create registrant tracking record per ministryInfo
		foreach ($ministryInfo as $thisMin) {
			if ( $this->save_registrant( $thisMin['uid'], $formvars ) ) {
				if ( ! $this->isDuplicate && ! $this->isWaitlist ) {
					$this->update_minopp_openings( $thisMin );
				}
			}
		}
...
//$contact['name'] = 0;
			...
			if ($formvars['youth'])		$emailBody .= $this->pi_getLL('form_youth_label.short','Youth?').': '.$this->pi_getLL('form_youth_label.yes')."\n\n";
			if ($formvars['parents_name'])	$emailBody .= $this->pi_getLL('parents_name','Parent/Guardian Name').': '.stripslashes($formvars['parents_name'])."\n\n";
			...
			if ($formvars['chosen_minopps']) {
				$emailBody .= $this->isCommitment ? $this->pi_getLL('email_makeCommitment','Is Committed To: ') : $this->pi_getLL('email_interestedIn','Is Interested In: ');
	...
	function display_signup_confirm($formvars) {
		...
		if ($formvars['youth'])   $signupText .= $this->pi_getLL('form_youth_label.short','Youth? ').': '.$this->pi_getLL('form_youth_label.yes')."<br>";
		if ($formvars['parents_name']) 	  $signupText .= $this->pi_getLL('form_parents_name_label','Parent/Guardian Name').': '.stripslashes($formvars['parents_name'])."<br><br>";
		...
		// MLC pass message through to user
		if ($this->responseMsg)
			$markerArray['###RESPONSE_MSG_TEXT###'] = $this->responseMsg;
		else
			$subpartMarkerArray['###SHOW_RESPONSE_MSG###'] = '';
	...
	function display_preview() {
		...
		$itemContent = "";
		$countMinOppArray	= count($minOppArray);
		for ($i = 0; $i < $countMinOppArray && $i < $this->config['preview_howmany']; $i++) {
	 ...
	 *
	 * @param	integer		$minOpp:	ministry opp uid signed up for
	...
	function save_tracking($minOpp, $userName, $contactUID, $inputArray) {
		...
		$newRec['crdate'] = $this->timeNow;
		$newRec['tstamp'] = $newRec['crdate'];
	...
	/**
	 * Save Registrant Information
	 *
	 * 		Will save to database registrant information of signup per ministry opportunity.
	 *
	 * @param	integer		$minOpp:	ministry opp uid signed up for
	 * @param	array		$inputArray: input form fields [name / data ]
	 * @return	integer		uid of record inserted (0 = failed)
	 */
	function save_registrant( $minOpp, $inputArray ) {
		$newRec['pid']			= $this->pid_list;
		$newRec['crdate']		= $this->timeNow;
		$newRec['tstamp']		= $newRec['crdate'];
		$newRec['minopp']		= $minOpp;
		$newRec['message']		= $inputArray['message'];
		$newRec['youth']		= ( $inputArray['youth'] || $this->isYouth ) ? 1 : 0;
		$newRec['parents_name']	= $inputArray['parents_name'];

		// MLC lookup or create user id
		$newRec['user_id']		= $this->get_userId( $inputArray );

		$newRec['waitlist']		= $this->isWaitlist ? $this->timeNow : 0;

		// MLC is registrant already registered or not
		// if registered, hide old, create new registration
		$this->isDuplicate		= $this->duplicateLookup( $newRec );

		$res					= $GLOBALS['TYPO3_DB']->exec_INSERTquery( 'tx_wecservant_registrant', $newRec );
		$insertWorked			= mysql_insert_id();

		if ( ! $insertWorked || mysql_error() )
			t3lib_div::debug( array( mysql_error(), $newRec ) );
	 ...
	 *
	 * Check tx_wecservant_registrant for similar entry. If found return id of it and hide old record.
	 *
	 * @param	array		$inputArray: input form fields [name / data ]
	 * @return	integer|boolean		id of duplicate entry; false for no duplicate
	 */
	function duplicateLookup( $inputArray ) {
		// fields to play with
		$pid					= $inputArray['pid'];
		$minOpp					= $inputArray['minopp'];
		$user_id				= $inputArray['user_id'];
		$where					= " pid = $pid AND minopp = $minOpp AND user_id = $user_id AND duplicate = 0";
		$where					.= ! $this->isWaitlist ? ' AND waitlist = 0' : ' AND waitlist > 0';

		// SELECT uid from tx_wecservant_registrant where pid, minopps, user_id match
		$row					= $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
			'uid',
			'tx_wecservant_registrant',
			$where
		);

		// if found, update tstamp uid tx_wecservant_registrant mark as duplicate
		// return uid
		if ( $row && count( $row ) ) {
			$res				= $GLOBALS['TYPO3_DB']->exec_UPDATEquery(
				...
				$where,
				array(
					// duplicate is a timestamp
					'duplicate'	=> $this->timeNow
				)
			);
	 ...
	 * Retrieve user's ID
	 *
	 * 		Will lookup or save to database registrants information as fe_user.
	 *
	 * @param	array		$inputArray: input form fields [name / data ]
	 * @return	integer		uid of record inserted or found (0 = failed)
	 */
	function get_userId( $inputArray ) {
		if ( $this->userID ) {
			return $this->userID;
		} else {
			return $this->lookup_userId( $inputArray );
		}
	}
	 ...
	 * Look up user's ID
	 *
	 * 		Will lookup by email or call save to database registrants information as fe_user.
	 *
	 * @param	array		$inputArray: input form fields [name / data ]
	 * @return	integer		uid of record inserted or found (0 = failed)
	 */
	function lookup_userId( $inputArray ) {
		// MLC lookup fe_user.uid by email
		$where				= "email = '" . $inputArray['email'] . "'";
		// $where				.= " AND pid = '" . $this->conf['userPID'] . "'";

		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'fe_users', $where);
		if ( mysql_error() ) {
			t3lib_div::debug( array( mysql_error(), "SELECT uid FROM fe_users WHERE ".$where ) );
		}

		if ( $row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res) ) {
			// MLC load pulled user to fe_user object for future recall
			$GLOBALS['TSFE']->fe_user->user	= $row;

			return $row['uid'];
		} else {
			return $this->create_userId( $inputArray );
		}
	}
	 ...
	 *
	 * 		Will save to database registrants information as fe_user.
	 *
	 * @param	array		$inputArray: input form fields [name / data ]
	 * @return	integer		uid of record inserted or found (0 = failed)
	 */
	function create_userId( $inputArray ) {
		$newRec['pid']			= $this->conf['userPID'];
		$newRec['crdate']		= $this->timeNow;
		$newRec['tstamp']		= $newRec['crdate'];

		// MLC TODO doesn't work for non-Western names like åº·æ¦®æ™º
		$nameArr				= explode( ' ', $inputArray['name'] );
		$newRec['first_name']	= array_shift( $nameArr );
		$newRec['last_name']	= implode( ' ', $nameArr );

		$newRec['name']			= $inputArray['name'];
		$newRec['email']		= $inputArray['email'];
		$newRec['telephone']	= $inputArray['phone'];

		$newRec['address']		= $inputArray['address'];
		$newRec['city']			= $inputArray['city'];
		$newRec['zone']			= $inputArray['state'];
		$newRec['zip']			= $inputArray['zip'];
		$newRec['usergroup']	= $this->conf['usergroupUID'];

		$res					= $GLOBALS['TYPO3_DB']->exec_INSERTquery( 'fe_users', $newRec );

		$insertWorked = mysql_insert_id();

		if ( ! $insertWorked || mysql_error() ) {
			t3lib_div::debug( array( mysql_error(), $GLOBALS['TYPO3_DB']->INSERTquery( 'fe_users', $newRec ) ) );
		}

		$newRec['uid']			= $insertWorked;

		// MLC load the newly created user to fe_user object for future recall
		$GLOBALS['TSFE']->fe_user->user	= $newRec;
	...
	/**
	 * Updates ministry opportunity openings count
	 *
	 * 		Will save to database ministry opportunity opening an
	 *
	 * @param	array		$minOpp:	array of ministry information
	 * @param	boolean		$decrement: true for decrement, false for increment
	 * @return	void
	 */
	function update_minopp_openings( $minOpp, $decrement = true ) {
		$newRec				= array();

		if ( $decrement ) {
			$newOpenings	= $minOpp['openings'] - 1;
		} else {
			$newOpenings	= $minOpp['openings'] + 1;
		}

		$newRec['openings']	= $newOpenings;
		$where				= "uid=".$minOpp['uid'];

		// MLC don't adjust openings when it's null
		$where				.= ' AND openings IS NOT NULL';
	...
	function getWECcookie($pid) {
		...
		if (isset($_COOKIE["WEC"]))	{
			...
			$countCData	= count($cData);
			for ($i = 0; $i < $countCData; $i++) {
	...
	function setWECcookie($pid,$data) {
		...
		$countCurCookies	= count($curCookies);
		for ($i = 0; $i < $countCurCookies; $i++) {
			$countCurCookiesi	= count($curCookies[$i]);
			for ($j = 0; $j < $countCurCookiesi; $j++)	{
				$newCookie .= ($j != (count($curCookies[$i]) -1)) ? $curCookies[$i][$j]."_" : $curCookies[$i][$j];
			}
			if ($i < (count($curCookies)-1)) $newCookie .= "|";
		}
		setcookie("WEC", $newCookie, $this->timeNow+($this->unixYear), '/');
	}
...
//		if (strlen($sortOrder) > 0) $sortOrder = ' ORDER BY '.$sortOrder;
	...
	function admin_menu() {
		...
		// MINISTRY OPP FORM
		$formFields = array('name','datetime','reference_code','ministry','contact','description','youth_friendly','supervision_required','location','times_needed','openings','priority','skills_needed','qualifications','misc_description');
	...
	function admin_addUpdMinOpp($passedInVars,$isEdit) {
		...
		elseif ($passedInVars['contact_uid'] && ($passedInVars['contact_uid'] != $passedInVars['original_contact_uid'])) {
		...
		elseif ($passedInVars['ministry_uid'] && ($passedInVars['ministry_uid'] != $passedInVars['original_ministry_uid'])) {
			$newRec['ministry_uid'] = $passedInVars['ministry_uid'];
		}

		// update tstamp for both new and updated (tstamp = last editted field)
		$newRec['tstamp'] = $this->timeNow;
	...
	function admin_addUpdContact($passedInVars,$isEdit) {
		...
		$newRec['tstamp'] = $this->timeNow; // update tstamp for both new and updated (tstamp = last editted field)
		...
		switch ($action) {
			...
			case 13: // = show inactive past all
				...
				for ($i = 0; $i < $countStats; $i++) {
					...
					$countCurMinOpps	= count($curMinOpps);
					for ($k = 0; $k < $countCurMinOpps; $k++) {
