...
/* $Id: class.tx_mthfeedit.php,v 1.1.1.1 2010/04/15 10:03:51 peimic.comprock Exp $ */
//Jaspreet Singh: This is a generic plugin for frontend editing that was modified adhoc just to get it
//working fast for Savvy Miss blogs.  There are some hacks.  You have been warned.
...
#      debug($GLOBALS['TCA_DESCR']);
	...
	// MLC set blogs
	$this->TCA["ctrl"]["title"]	= "Blog Entry";
	#    debug($this->TCA,'TCA');
...
#      debug(is_object($GLOBALS['TBE_TEMPLATE']));      
  ...
  function makeHTMLPreview($cmd, $withHTML = true) {
	...
	if($this->conf[$cmd.'.']['preview.']['dontDisplayEmptyFields']   // if dontDisplayEmptyFields is set, skib the field
	   || ($cmd == 'all' && ($this->conf['create.']['preview.']['dontDisplayEmptyFields'] || $this->conf['edit.']['preview.']['dontDisplayEmptyFields']))) {  
	  continue;
	}
      }
...
#    $result[] = ''.implode(chr(10),$hiddenFields);
    ...
    //jsmodified
	$implodedResult = implode(chr(10),$result);
	
	// Keep callers piVars
    if($withHTML) {
		return $this->addCallersPiVars($implodedResult); 
	}
	else { 
		$plainText = $this->htmlToPlainText($implodedResult);
		return $plainText;
	 }
...
#	$whereClause = str_replace('###THIS_CID###',,$whereClause);
	...
	if ($GLOBALS['TYPO3_DB']->sql_error())	debug(array($GLOBALS['TYPO3_DB']->sql_error()),'getFormFieldCode()::field='.$fN);
...
<!-- ###TEMPLATE_EDITMENU### begin -->
	<br />'.($this->conf['no_header']?'':'<h1 class="'.$this->caller->pi_getClassName('header').' '.$this->caller->pi_getClassName('header-editmenu').'">'.$this->getLL("edit_menu_header").'</h1>').'
...
<div'.$this->caller->pi_classParam('form-row').'>
   ###HIDDENFIELDS###
   <input type="Submit" name="submit" value="'.($this->conf['edit.']['preview']?$this->getLL("edit_submit_label"):$this->getLL("edit_preview_submit_label")).'"'.$this->caller->pi_classParam('form-submit').'>
</div>
</form>
...
<div'.$this->caller->pi_classParam('form-row').'>
   ###HIDDENFIELDS###
   <input type="Submit" name="submit" value="'.($this->conf['create.']['preview']?$this->getLL("create_submit_label"):$this->getLL("create_preview_submit_label")).'"'.$this->caller->pi_classParam('form-submit').'>
</div>
</form>
</div>
...
//jsmodified: removed .nl2br($this->makeTEXTPreview('all')).
return '
<!-- ###TEMPLATE_SETFIXED_OK### -->
'.($this->conf['no_header']?'':'<h1 class="'.$this->caller->pi_getClassName('header').' '.$this->caller->pi_getClassName('header-setfixed-ok').'">Entry Approved</h1>').'
<div class="'.$this->caller->pi_getClassName('message').' '.$this->caller->pi_getClassName('message-setfixed-ok').'">Entry "###FIELD_'.$this->id_field.'###", has been approved.<br/><br/>' . '</div>
<!-- ###TEMPLATE_SETFIXED_OK### end-->

<!-- ###TEMPLATE_SETFIXED_OK_DELETE### -->
'.($this->conf['no_header']?'':'<h1 class="'.$this->caller->pi_getClassName('header').' '.$this->caller->pi_getClassName('header-setfixed-ok-delete').'">Entry Deleted</h1>').'
<div class="'.$this->caller->pi_getClassName('message').' '.$this->caller->pi_getClassName('message-setfixed-ok').'">Entry "###FIELD_'.$this->id_field.'###", has been deleted.<br/><br/>'.'</div>
...
';
  ...
  function getEmailTemplate() {
	//echo $this->makeTEXTPreview('edit'); //jstmp
	//jsmodified
	$plainText = preg_replace( '/([a-z0-1]###)/', "$1\n", $this->makeTEXTPreview('all') );
	// echo $plainText;
    return '
<!-- ###EMAIL_TEMPLATE_CREATE_SAVED### begin -->
Savvy Miss: New Blog Entry

	<!--###SUB_RECORD###-->
        You have submitted the following blog entry at '.t3lib_div::getIndpEnv('TYPO3_SITE_URL').':'.chr(10).'
	'.$this->makeTEXTPreview('all').'
	<!--###SUB_RECORD###-->
...
Savvy Miss: New Blog Entry 
	...
	<!--###SUB_RECORD###-->'
	//. /*jsmodified*/ $this->htmlToPlainText($this->makeTEXTPreview('all')).
	//. $this->htmlToPlainText($this->makeTEXTPreview('edit')).
	//. $this->htmlToPlainText('<p>I gotta say Im a</p>').
	//.$plainText.
	.$this->makeTEXTPreview('all').
'	
Approve:
...
';
  ...
  function processDataArray($content,$conf) {
	//echo "function processDataArray($content,$conf) {"; //tmpjs
    //debug($content,'processDataArray content');//TODODEBUG
    //debug($conf,   'processDataArray conf');//TODODEBUG
	...
	if(($GLOBALS['TCA'][$table]['columns'][$fN]['config']['eval']=='author'  &&
	   $fe_adminLib->conf[$fe_adminLib->cmdKey."."]["overrideValues."][$fN] == '[currentUsername]')) {
		   //echo '[currentUsername] block ';//jstmp
...
//	  echo "function afterSave($content,$conf)	{" ; //jstmp
#    debug($content,'content');
#    debug($conf,'conf');
#    debug($_FILES,'$_FILES');
		...
		if(( 
		   $fe_adminLib->conf[$fe_adminLib->cmdKey."."]["overrideValues."][$fN] == '[currentUsername]')) {
			   ...
			   if ($GLOBALS['TSFE']->loginUser) {
				   ...
				   //echo "uid : " . $insertID; //$dataArr['uid'];
				   
				   $query = "
						UPDATE fe_users, tt_news
						SET tt_news.author = fe_users.username
						WHERE 
						fe_users.uid = tt_news.tx_newsfeedit_fe_cruser_id
						AND tt_news.uid = $insertID
						";
						
				   //copy the user image from users upload dir to news upload dir - js
					$documentRoot =  $_SERVER['DOCUMENT_ROOT'];
					//echo "documentRoot: $documentRoot"; //jsdbg
					
					$uploadsDir = $documentRoot . '/fileadmin/uploads';
					$userPicsDir = $uploadsDir . '/tx_srfeuserregister';
					#$ttNewsUsersPicsDir = $uploadsDir . '/pics/feusers';
					#$ttNewsUsersPicsDirPrefix = 'pics/feusers/';
					$ttNewsUsersPicsDir = $uploadsDir . '/pics';
					$ttNewsUsersPicsDirPrefix = '';
					$userImage = $GLOBALS['TSFE']->fe_user->user['image'];
					$slash = '/';
					$source = $userPicsDir . $slash . $userImage; 
					$destination = $ttNewsUsersPicsDir . $slash . $userImage;
					
					//echo "<br>\nCopying $source to $destination."; //jsdbg
					
					if (!empty($userImage)) {
						copy( $source, $destination );
						//resize the image, just in case
						$maxImageSizeW = '117';
						$maxImageSizeH = '156';
						$esapedParam = escapeshellcmd($destination);
						//resize to max of specified height and width
						$commandLine = "mogrify -resize {$maxImageSizeW}x{$maxImageSizeH} $esapedParam";
						//echo $commandLine; //jstmp;
						system($commandLine);

					}
				   
					#$userImageRelativePath = $ttNewsUsersPicsDirPrefix . $userImage;
					$userImageRelativePath = $userImage;
				   // MySQL 3. blows
				   $query = "
						UPDATE tt_news
						SET tt_news.author = '{$GLOBALS['TSFE']->fe_user->user['username']}'
						, tt_news.image = '$userImageRelativePath'
						WHERE tt_news.uid = $insertID
						";
