...
$Id: LinkpointCreditCard.class.php,v 1.1.1.1 2010/04/15 10:04:04 peimic.comprock Exp $
...
class LinkpointCreditCard {
	...
	*/
	function preAuth($cc_number, $cc_month, $cc_year, $chargetotal
		, $userArr
        , $details
	)
	{
		if ($this->debug) {
			echo "function preAuth($cc_number, $cc_month, $cc_year, $chargetotal) {";
		}
		if ($this->testing) {
		...
		if ($result["r_approved"] != "APPROVED") // transaction failed, print the reason
		{
			...
			$subjectLine = "BPM: LinkPoint failure in preAuth";
			...
			mail( 'michael@peimic.com'
				...
				, 'file ' . __FILE__
					. "\nline: " . __LINE__ 
					. "\nresult" . cbPrintString( $result )
					. "\nthis" . cbPrintString( $this )
					. "\nmylphp" . cbPrintString( $mylphp )
					. "\nmyorder" . cbPrintString( $myorder )
			);
			}
		}
		else
		{ // success
			if ($this->debug) {
				echo "Status: $result[r_approved]\n";
				echo "Code: $result[r_code]\n";
				echo "OID: $result[r_ordernum]\n\n";
			}
	...
	*/
	...
	)
	{
		...
		if ($result["r_approved"] != "APPROVED") // transaction failed, print the reason
		{
			...
			$subjectLine = "BPM: LinkPoint failure in postAuth";
			if ($this->debug) { $subjectLine .= " (\$debug is on)"; }
			if ( true )
			{
			mail( 'michael@peimic.com'
