...
*
* TYPO3 CVS ID: $Id: class.tx_srfeuserregister_pi1.php,v 1.1.1.1 2010/04/15 10:04:05 peimic.comprock Exp $
*
*/
...
class tx_srfeuserregister_pi1 extends tslib_pibase {
		...
		function main($content, $conf) {
			...
			$this->sys_language_content = intval($TSFE->config['config']['sys_language_uid']) ? intval($TSFE->config['config']['sys_language_uid']) : 0;
			...
			// echo "this->TCA"; print_r($this->TCA);//jsdebug
			...
			$this->pidRecord->sys_language_uid = $this->sys_language_content;
			...
			$this->registerPID = intval($this->conf['registerPID']) ? strval(intval($this->conf['registerPID'])) : $TSFE->id;
			$this->editPID = intval($this->conf['editPID']) ? strval(intval($this->conf['editPID'])) : $TSFE->id;
			...
			$this->cmd = $this->feUserData['cmd'] ? $this->feUserData['cmd'] : $this->cObj->caseshift($this->cObj->data['select_key'],'lower');
			...
			if ($this->theTable == 'fe_users' && $TSFE->loginUser && $this->cmd != 'invite') {
			...
			if ($this->saved) {
				...
				) 
				{
					...
					header('Location: '.t3lib_div::locationHeaderUrl($this->site_url.$this->cObj->getTypoLink_URL($this->loginPID.','.$GLOBALS['TSFE']->type,
	$loginVars)));
					exit;
				}
				...
				switch($this->cmd) {
					...
					case 'create':
						...
						break;
					default:
						if ($this->theTable == 'fe_users' && $TSFE->loginUser) {
							$content = $this->displayCreateScreen($this->cmd);
						} else {
							$content = $this->displayEditScreen();
						}
		...
		*/
		function displayEditForm($origArr) {
			...
			$markerArray['###HIDDENFIELDS###'] .= chr(10) . '<input type="hidden" name="FE['.$this->theTable.'][tx_smcustomizations_terms_agree]" value="on" />';
			if ($this->theTable != 'fe_users') {
		...
		*/
		function displayEditScreen() {
			...
			if ($this->conf['edit']) {
				...
				$origArr = $TSFE->sys_page->getRawRecord($this->theTable, $this->dataArr['uid']?$this->dataArr['uid']:$this->recUid);
				...
				if (is_array($origArr) && ( ($this->theTable == 'fe_users' && $TSFE->loginUser) || $this->aCAuth($origArr) || !strcmp($this->authCode, $theCode) ) ) {
		...
		*/
		function setfixedHash($recCopy, $fields = '') {
			...
			$authCode = substr(md5($authCode), 0, $this->codeLength);
		...
		*/
		function addLabelMarkers($markerArray, $dataArray) {
			...
			while (list(, $labelName) = each($otherLabels) ) {
				$markerArray['###LABEL_'.$this->cObj->caseshift($labelName,'upper').'###'] = sprintf($this->pi_getLL($labelName), $this->thePidTitle, $dataArray['username'], $dataArray['name'], $dataArray['email'], $dataArray['password']); 
			}
			return $markerArray;
		}
		/**
		...
		*/
		function addFileUploadMarkers($theField, $markerArray, $dataArr = array(), $viewOnly = false) {
			...
			if ($this->previewLabel || $viewOnly) {
				...
				if ($dataArr[$theField]) {
					$label = $this->pi_getLL('cur'.$theField);
					if ($label) $markerArray['###LABEL_'.$this->cObj->caseshift($theField,'upper').'###'] = $label;
				}
			}
			return $markerArray;
		}
		...
		function buildFileUploader($fName, $config, $filenames = array(), $prefix, $viewOnly = false) {
			...
			if ($this->previewLabel || $viewOnly) {
				...
				for ($i = sizeof($filenames); $i < $number + sizeof($filenames); $i++) {
					//js reduce size of box
					$HTMLContent .= '<input id="'.
					$this->pi_getClassName($fName) . '-' .
					($i-sizeof($filenames)) . '"
					name="'.$prefix.'['.$fName.']['.$i.']'.'" title="' .
					$this->pi_getLL('tooltip_' . (($this->cmd ==
					'invite')?'invitation_':'')  . 'image') . '" size="25" type="file" '.$this->pi_classParam('uploader').' /><br />';
				}
			}
