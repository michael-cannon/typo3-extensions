	...
	function curl_process($data)
	{
		...
		if (isset($data["cbin"])) //using BINARY curl methods
		{
			if ($data["cbin"] == "true")
			{
				...
				{
					$result = "<r_approved>p255 FAILURE</r_approved><r_error>Could not connect.</r_error>"; 
					return $result;
				}
...
#			curl_setopt ($ch, CURLOPT_SSL_VERIFYPEER, 0);
			...
			if (strlen($result) < 2)    # no response
			{
				$result = "<r_approved>p304 FAILURE</r_approved><r_error>Could not connect.</r_error>"; 
				return $result;
			}
