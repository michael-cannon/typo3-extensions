...
#  var $LANG; // language object, used to translate 'complicated' label strings to the single label.
  ...
  function init($caller,$conf)	{
	...
	$GLOBALS[$this->prefixId]['isOur'] = false; //jsdebug
...
#      debug($GLOBALS['TCA_DESCR']);
	...
	//echo " ftw1: " .$foreign_table_where; //jsdebug
    ...
    //echo " ftw2: " . $GLOBALS['TCA']['tt_news']['columns']['category']["config"]["foreign_table_where"]; //jsdebug
	//if($GLOBALS[$this->prefixId]['isOur'] == true) //jsdebug
	if(true)
    {
    	if(!isset($GLOBALS['TCA']['tt_news']['columns']['category']["config"]["foreign_table_where"])) {
			...
			//echo "; set new val = " . $GLOBALS['TCA']['tt_news']['columns']['category']["config"]["foreign_table_where"];
		}
    }
    // >>> td;
...
#			debug(is_object($GLOBALS['TBE_TEMPLATE']));
			...
			if($fieldCode)
			{
				...
				$bodytextExplanation =  ('bodytext'==$fN) ? '<p>To insert an image, drag it from another website browser window into the text area above where you want the image to appear. Alternatively, upload an image file from you computer with the "Browse" button below.</p>' : '' ; //jsmodified
				$out_array[$out_sheet][]='<div class="'.$this->caller->pi_getClassName('form-row').' '.$this->caller->pi_getClassName('form-row-'.$fN).'">
					...
					<div'.$this->caller->pi_classParam('form-field').'>'.$fieldCode.$bodytextExplanation.'</div>
					'.$msg.'
					</div>';
			}
		}
   ...
   * @param	string		The cmd. Should be 'edit' or 'create' or 'all' or 'email'
  ...
  function makeHTMLPreview($cmd, $withHTML = true) {
	...
	/*
	The fields assignation statement was ridiculously complex, so I split
	it up into multple statements.  In addition, the command 'all' was being
	used for email templates, but that pulls in more fields than desired.
	
	So now we use the fields from conf email.field
	
	Jaspreet Singh 
	*/
	//get the union of the fields, from the create or edit show_fields, or both 
	$rawFieldList = $this->conf['create.']['show_fields'] 
			? $this->conf['create.']['show_fields'] . (
				$this->conf['edit.']['show_fields']
				? ','.$this->conf['edit.']['show_fields']
				:''
			) 
			: $this->conf['edit.']['show_fields']
	;
	$fields = (string)$cmd=='all' 
		? array_unique(t3lib_div::trimExplode(",",($rawFieldList))) 
		: array_unique(t3lib_div::trimExplode(",",$this->conf[$cmd.'.']['show_fields']))
	;
	
	//For emails, don't get all fields, just ones specified in the admin email field list
	if ($cmd='email') {
		$fields = array_unique(t3lib_div::trimExplode(",",$this->conf['email.']['field']));
	}
    $result = array();
...
//jsdebug
//echo "function getPreviewFieldCode fN:$fN ";
...
#    debug(intval($feData[$this->table]['datetime']),'datetime');
  ...
  {
//jsdebug 
//echo "function getFormFieldCode fN:$fN "; 
...
<br />
	...
	<a href="###THIS_URL######FORM_URL######SYS_SETFIXED_approve###">Approve</a>
<br />
<br />
	<a href="###THIS_URL######FORM_URL######SYS_SETFIXED_DELETE###">Delete</a>
	<!--###SUB_RECORD###-->
</html>
...
';
  ...
  function processDataArray($content,$conf) {
    //echo "function processDataArray ";
	//echo "processDataArray content $content"; //jsdebug
    //echo "processDataArray conf $conf"; //jsdebug
    ...
    foreach((array)$dataArr as $fN=>$value) {
		...
		//add "By: " (4 characters) 
		//to the beginning of the author field.
		if ('author'==$fN) {
				//echo "author: " . $dataArr[$fN]; //jsdebug
				//only add the prefix if we haven't already added the prefix
				$byPrefix = 'By: ';
				$author = $dataArr[$fN];
				if( substr($author,0,4) != $byPrefix ) {
					$dataArr[$fN] = $byPrefix . $author;
				}
		}
	...
	//Modification by Jaspreet Singh
	//$dataArr//html_entity_decode()
	//echo 'processDataArr:';echo $fN; //jsdebug
	//echo $dataArr[$fN];
	//$dataArr[$fN] = html_entity_decode($dataArr[$fN]);
	$dataArr[$fN] = htmlentities2utf8($dataArr[$fN]);
	//$dataArr[$fN] = htmlentities2asciib($dataArr[$fN]);
	//echo "\n<br \>";
	//echo $dataArr[$fN];
  ...
  function updateArray($content,$conf) {
	//echo "function updateArray "; //jsdebug
	//echo "content:"; print_r($content) ;
	//echo "conf:"; print_r($conf);
...
';
  ...
  function rteProcessDataArr($dataArr, $table, $fN, $mode) {
    //echo "function rteProcessDataArr ";//return $dataArr; //jsdebug
	if(t3lib_extmgm::isLoaded('rtehtmlarea') && !$this->RTEObj)
      $this->RTEObj = t3lib_div::makeInstance('tx_rtehtmlarea_pi2');
//echo"\$dataArr0:";print_r( $dataArr);//jsdebug
...
# THIS WILL NOT WORK YET! mail sendt to author of rtehtmlarea STAN
      ...
      $this->thisConfig = $pageTSConfig['RTE.']['default.']['FE.'];
//	}
	//echo"\$dataArr1:";print_r( $dataArr);//jsdebug
      $this->thePidValue = $GLOBALS['TSFE']->id;
#	  $this->specConf = array('richtext' => 1, 'rte_transform' => array('parameters' => array('mode=ts_css','flag=rte_enabled')));
	  ...
	  $textWithSpaces =  $this->crToSpaceCR($dataArr[$fN]);
	  $dataArr[$fN] = $textWithSpaces;
	  $dataArr[$fN] = $this->RTEObj->transformContent($mode,$dataArr[$fN],$table,$fN,$dataArr,$this->specConf,$this->thisConfig,'',$this->thePidValue);
	  //jsmodified: add spaces to the ends of lines, since transformContent seems to 
	  //neglect to do so. Jaspreet Singh
	  
	  
    }
	//echo"\$dataArr2:";print_r( $dataArr);//jsdebug
  ...
  function array_merge_recursive2($paArray1, $paArray2) {
	...
	/**
	Takes a string which contains lines delimited with CR/LF (carriage return--
	ASCII 13,	and line feed--ASCII 10) and replaces the CR with a space
	followed by a CR.
	The reason for this is as a hack because something or another in the
	processing for transformContent conjoins lines without adding a space
	between lines.
	@param string the string in which to append spaces
	@author Jaspreet Singh
	*/
	function crToSpaceCR($str) {
		//echo "crToSpaceCR "; //jsdebug
		$CR = "\r";
		$space = " \r";
		$textWithSpaces = str_replace ( $CR, $space, $str );
    ...
    {
        $string = html_entity_decode($string); //js: decode all named entities first.
		$string = preg_replace_callback('~&(#(x?))?([^;]+);~', 'html_entity_replace', $string);
        return $string;
    }

    function htmlentities2asciib ($string) // because of the html_entity_decode() bug with UTF-8
    {
        echo "htmlentities2asciib ";
		//js: convert all numeric entities to the corresponding ASCII code
		$string = t3lib_div::deHSCentities($string); 
		//js: decode all named entities first.
		$string = html_entity_decode($string);
		$string = ereg_replace('&([#[:alnum:]]*;)','&\1',$string);
        return $string;
    }
