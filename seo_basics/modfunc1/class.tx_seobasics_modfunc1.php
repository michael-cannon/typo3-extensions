...
class tx_seobasics_modfunc1 extends t3lib_extobjbase {
	...
	function renderSEOTable($tree) {
		...
		return '
		<table border="0" cellspacing="1" cellpadding="0" id="tx-seo_basics" class="lrPadding c-list">'.$output.'
		</table>';
	}
	...
	function renderRowContent($item, $rowTitle = NULL) {
		...
		if ($cmd != 'edit') {
			...
			$row1[] = '<td>Title-Tag:</td><td><input name="'.$fName.'[tx_seo_titletag]" value="'.$item['tx_seo_titletag'].'" type="text" size="43" maxlength="100" autocomplete="off" class="seoTitleTag"/></td><td>Keywords:</td><td><input name="'.$fName.'[keywords]" value="'.$item['keywords'].'" type="text" size="67" maxlength="180" autocomplete="off" class="seoKeywords"/></td>';

			$row2[] = '<td>Description:</td><td colspan="3"><input name="'.$fName.'[description]" value="'.$item['description'].'" type="text" size="120" autocomplete="off" class="seoDescription"/><br/><br/></td>';
			$row2[] = ''.$fields.'';
		}
