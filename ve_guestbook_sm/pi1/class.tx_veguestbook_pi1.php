...
#require_once(t3lib_extMgm::extPath('tx_commentnotify_pi1').'pi1/class.tx_commentnotify_pi1.php');
#echo t3lib_extMgm::extPath('tx_commentnotify_pi1').'pi1/class.tx_commentnotify_pi1.php';
...
class tx_veguestbook_pi1 extends tslib_pibase {
	...
	function getListContent($res, $templatecode) {
		...
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			...
			// - js remove image area if not needed
			if (empty($markerArray['###GUESTBOOK_IMAGE###'])) {
				$current_templatecode = $this->cObj->substituteSubpart($current_templatecode,'###ENTRY_IMAGE###','');
			}
	...
	function getItemMarkerArray($row) {
		...
		//get image - js
		$feRow = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
			'uid, image, username',
			'fe_users',
			"uid = '" . $row['fe_userid'] . "'",
			'',
		...
		$feRowQuery = $GLOBALS['TYPO3_DB']->SELECTquery(
			'uid, image',
			'fe_users',
			"uid = '" . $row['fe_userid'] . "'",
			'',
		...
		//print_r($feRow); echo $feRowQuery; //tmp debug
		if (is_array($feRow)) {
			$markerArray['###GUESTBOOK_IMAGE###'] = $feRow[0]['image'];
			$markerArray['###GUESTBOOK_FIRSTNAME###'] = $feRow[0]['username'];
			//echo 'GUESTBOOK_IMAGE:' . $feRow[0]['image'] . ' ' ;
		}
	...
	function displayForm() {
		...
		if (is_array($this->LOCAL_LANG) && count($this->LOCAL_LANG) > 0)
		{
			...
			// << YA; Insert Get parameters of paging in form action; Jun 2006;
			...
			if ($this->postvars['submitted'] == 1) {
				...
				if (!empty($error)) {
					...
					if ($GLOBALS['TSFE']->loginUser) {
						$saveData['fe_userid'] = $GLOBALS['TSFE']->fe_user->user['uid'];
					}
			...
			if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ve_guestbook']['preEntryInsertHook'])) {
				foreach($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ve_guestbook']['preEntryInsertHook'] as $_classRef) {
					...
					if ($insert) {
						...
						//Jaspreet Singh: Added call to queue notifications for the other posters in this thread.
						//Do only if we're commenting on a news article, and there is a frontend user logged in.
						if (true && ($this->for_tt_news) && ($GLOBALS['TSFE']->loginUser) ) {
							//echo 'Queuing notifications. '; //jsdebug 
							$newsID = $saveData['uid_tt_news'];
							$lastPosterFeID = $saveData['fe_userid'];
							...
							$cn->processForNewsComment( $newsID, $lastPosterFeID, $internalUrl);
							
						}

						header('Location: '.t3lib_div::getIndpEnv('TYPO3_REQUEST_DIR').$this->getUrl($this->config['redirect_page']));
						// << YA; Redirect to the same page; Jun 2006;
            ...
            if (!$this->postvars && $GLOBALS['TSFE']->loginUser) {
					...
					$markerArray['###VALUE_IMAGE###'] = $GLOBALS['TSFE']->fe_user->user['image'];
            }
