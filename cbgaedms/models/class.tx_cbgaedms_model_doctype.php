...
/**
 * Class that implements the model for table tx_cbgaedms_doctype.
 *
...
class tx_cbgaedms_model_doctype extends tx_cbgaedms_model_common {
	function tx_cbgaedms_model_doctype($controller = null, $parameter = null) {
	...
	function load($parameters = null) {
		// fix settings
		$fields = '*';
		$tables = 'tx_cbgaedms_doctype t';
		$groupBy = null;
		$orderBy = 'doctype ASC';
		$limit = null;
		$where = 't.hidden = 0 AND t.deleted = 0 ';
		$where .= ' AND t.agency = ' . $this->controller->configurations->get('masterAgency');

		// variable settings
		if($parameters) {
			// do query modifications according to incoming parameters here.
			// don't allow for result
			if ( true === $parameters->get('doctype') && ! $parameters->get('uid') )
				$where .= ' AND 1 = 0';
	...
	function preparedParameters ( $insert = false ) {
		...
		foreach($parameters->selectHashArray('doctype, description, required, hidden') as $key => $value) {
			$parameterArray[$key] = htmlspecialchars(trim($value));
		}
		$parameterArray['required'] = ('' == $parameterArray['required'])
									? 0
									: 1;
	...
	function update() {
		...
		$query = $GLOBALS['TYPO3_DB']->UPDATEquery('tx_cbgaedms_doctype', $where, $updateArray);
		$GLOBALS['TYPO3_DB']->sql_query($query);
	}

	function insert() {
		$insertArray = $this->preparedParameters(true);
		$query = $GLOBALS['TYPO3_DB']->INSERTquery('tx_cbgaedms_doctype', $insertArray);
		$GLOBALS['TYPO3_DB']->sql_query($query);
	}

	function loadDoctypesAsOptions($parameters = null) {
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cbgaedms/models/class.tx_cbgaedms_model_doctype.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cbgaedms/models/class.tx_cbgaedms_model_doctype.php']);
}
