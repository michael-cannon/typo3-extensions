function f1(tdr){
	tdr.className='menuborder test1';
}
function f2(tdr){
	tdr.className='menuborder test';
}
...
/**
 * Modified By       : Chetan Thapliyal
 * Modification Date : 22 Dec 2005
 * Description       : Made possible to select current
 *                     date in calendar
 */
function pickDate(d) {
  hideCal();
  ...
  var res = compareDates(getCurrentDate(),date);
  //if( (res == 1) || (res ==0)) {
  if( (res == 1)) {
     // If selected date is greater than or equals to current date
     ...
     // Call user function to further process processing
     // depending upon the selected date
     if ( dateChangeEventHandler) {
        dateChangeEventHandler(date);
     }
  }
}
// ------
...
//addCalendar("Date", "Select Date", "opDate", "joblisting");
//addCalendar("Date2", "Select Date", "clDate", "joblisting");
//
...
function mOver(Component,option)
{
	var fColor=option[0];
	var bColor=option[1];
	Component.bgColor=bColor;
}
