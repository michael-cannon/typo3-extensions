...
class tx_tfclifegroups_pi1 extends tslib_pibase {
	var $prefixId = 'tx_tfclifegroups_pi1';		// Same as class name
	var $scriptRelPath = 'pi1/class.tx_tfclifegroups_pi1.php';	// Path to this script relative to the extension dir.
	var $extKey = 'tfc_lifegroups';	// The extension key.
	var $pi_checkCHash = TRUE;
	var $arDays = array();
	var $arAges = array();
	var $arInterests = array();
	var $arRecurrences = array();
	...
	function messageView($content,$conf) {
		if($this->templateFileContent){
			$dataRowContent = $this->cObj->getSubpart($this->templateFileContent,'###MESSAGE###');
	...
	function pi_list_header()	{
		if($this->templateFileContent){
			...
			#t3lib_div::debug($this->strHead, '$this->strHead');
			#t3lib_div::debug($this->strFoot, '$this->strFoot');
			$this->strFoot = str_replace('</p><table><tr><td', '<table><tr><td', $this->strFoot);
			$this->strFoot = str_replace('</td></tr></table></div>', '</td></tr></table>', $this->strFoot);
			#t3lib_div::debug($this->strFoot, '$this->strFoot (cleaned)');
			
			$htmlParse = "";
			$dataRowContent = $this->cObj->getSubpart($this->templateFileContent,'###RESULTS_HEAD###');
			$subStrArray = Array();
			#$subStrArray['###FOOTER###'] = $this->strFoot;
			#$htmlParse .= $this->strFoot;
			$htmlParse .= $this->cObj->substituteMarkerArrayCached($dataRowContent,$subStrArray);
			return $htmlParse;
		} else {
			return "Template not found";
	...
	function setDays() {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			'title, uid',
			'tx_tfclifegroups_days',
			'1'.$this->cObj->enableFields("tx_tfclifegroups_days"),
			'',
			'uid');
	...
	function setInterests() {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			'title, uid',
			'tx_tfclifegroups_interests',
			'1'.$this->cObj->enableFields("tx_tfclifegroups_interests"),
			'',
			'uid');
	...
	function setAges() {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			'title, uid',
			'tx_tfclifegroups_ages',
			'1'.$this->cObj->enableFields("tx_tfclifegroups_ages"),
			'',
			'uid');
	...
	/*
	 * @return array of recurrences
	 */
	function setRecurrences() {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			'descr, uid',
			'tx_tfclifegroups_recurrences',
			'1'.$this->cObj->enableFields("tx_tfclifegroups_recurrences").$pidList,
			'',
			'uid');
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			$arRecurrences[$row['uid']] = $row['descr'];
		}	

		return $arRecurrences;	
	}
	...
	function setCategories() {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			'title, uid',
			'tx_tfclifegroups_categories',
			'1'.$this->cObj->enableFields("tx_tfclifegroups_categories"),
			'',
			'uid');
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			$arCategories[$row['uid']] = $row['title'];
		}	
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tfc_lifegroups/pi1/class.tx_tfclifegroups_pi1.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tfc_lifegroups/pi1/class.tx_tfclifegroups_pi1.php']);
}
