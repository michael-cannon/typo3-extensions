...
***************************************************************/
/** 
 * Module extension (addition to function menu) 'Leads Download' for the 'news_lead' extension.
 *
 * @author	Michael Cannon <michael@peimic.com>
 * @version $Id: class.tx_newslead_modfunc1.php,v 1.1.1.1 2010/04/15 10:03:55 peimic.comprock Exp $
 */
...
class tx_newslead_modfunc1 extends t3lib_extobjbase
{
	...
	function main()
	{
		...
		if ( t3lib_div::_GP( 'tx_newslead_modfunc1_download' ) )
		{
			$this->doDownload();
		}

		// check for display request
		elseif ( t3lib_div::_GP( 'tx_newslead_modfunc1_display' ) )
		{
			$this->doDisplay();
		}
	...
	function doDownload ()
	{
		...
		// cbPrint2( 'membersCsv', $membersCsv );
	...
	function sqlParts ()
	{
		...
		if( ! t3lib_div::_GP( 'tx_newslead_modfunc1_summary' ) )
		{
			$select				= "
				SELECT DISTINCT
		...
		$startdate				= t3lib_div::_GP(
									'tx_newslead_modfunc1_startdate'
								);
		$where					.= ( $startdate )
									? ' AND l.crdate >= '
										. strtotime( $startdate )
		...
		$newscategories			= t3lib_div::_GP(
									'tx_newslead_modfunc1_newscategories'
								);
		...
		$filenames				= t3lib_div::_GP(
									'tx_newslead_modfunc1_filenames'
								);
	...
	function downloadOptionForm ()
	{
		...
		$input					= '
			<input type="checkbox" name="tx_newslead_modfunc1_summary"
				value="1" ' . ($this->getPreviousValue('tx_newslead_modfunc1_summary') == 1 ? ' checked ' : ''). '/>
		';
		$input					.= $LANG->getLL( 'summary' )
									. '<br />';

		$input					.= '
			<input type="checkbox" name="tx_newslead_modfunc1_nonsent"
				value="1" ' . ($this->getPreviousValue('tx_newslead_modfunc1_nonsent') == 1 ? ' checked ' : '') . ' />
		';
		...
		$submit					= '<input type="submit"
										name="tx_newslead_modfunc1_download"
										value="'
									. $LANG->getLL( 'submit_download' )
									. '" />';
		$submit					.= '&nbsp;';
		$submit					.= '<input type="submit"
										name="tx_newslead_modfunc1_display"
										value="'
...
//									. $user
	...
	{
		$string					= '
			<select name="tx_newslead_modfunc1_users[]" multiple="multiple"
		...
		{
			$string				.= '<option'
									. $this->selectIfPreviouslySelected('tx_newslead_modfunc1_users', $data['uid'])
									.' value="' . $data[ 'uid' ] . '">'
									. substr( $data[ 'name' ]
										, 0
										, $this->maxTitleLength
	...
	{
		$elementName = 'tx_newslead_modfunc1_' . $field;
	...
	{
		$string					= '
			<select name="tx_newslead_modfunc1_filenames[]" multiple="multiple"
				size="' . $this->selectSize . '">
		';
		...
		{
			$string				.= '<option'
									. $this->selectIfPreviouslySelected('tx_newslead_modfunc1_filenames', $data['filename'])
									.' value="' . $data[ 'filename' ]
									. '">'
	...
	{
		$string					= '
			<select name="tx_newslead_modfunc1_sponsors[]" multiple="multiple"
				size="' . $this->selectSize . '">
		';
		...
		{
			$string				.= '<option'
									. $this->selectIfPreviouslySelected('tx_newslead_modfunc1_sponsors', $data['uid'])	
									. ' value="' . $data[ 'uid' ] . '">'
									. substr( $data[ 'title' ]
										, 0
										, $this->maxTitleLength
	...
	{
		$string					= '
			<select name="tx_newslead_modfunc1_newscategories[]"
		...
		{
			$string				.= '<option' 
									. $this->selectIfPreviouslySelected('tx_newslead_modfunc1_newscategories', $data['uid'])						
									. ' value="' . $data[ 'uid' ] . '">'
									. substr( $data[ 'title' ]
										, 0
										, $this->maxTitleLength
	...
	{
		$string					= '
			<select name="tx_newslead_modfunc1_news[]" multiple="multiple"
				size="' . $this->selectSize . '">
		';
		...
		{
			$string				.= '<option  ' 
									. $this->selectIfPreviouslySelected('tx_newslead_modfunc1_news', $data['uid'])
									. 'value="' . $data[ 'uid' ] . '">'
									. substr( $data[ 'title' ]
										, 0
										, $this->maxTitleLength
	...
	*/
	function setDefaultFormValues() {
		...
		if (count($_POST)==0) {
			...
			//Set default values if required in the following way:
			/*
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/news_lead/modfunc1/class.tx_newslead_modfunc1.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/news_lead/modfunc1/class.tx_newslead_modfunc1.php']);
}
