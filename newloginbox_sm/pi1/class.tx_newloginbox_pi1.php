 ...
 *
 * $Id: class.tx_newloginbox_pi1.php,v 1.1.1.1 2010/04/15 10:03:54 peimic.comprock Exp $
 * XHTML compliant!
 *
	...
	function main($content,$conf)	{
		...
		if ($this->piVars['forgot'])	{
			...
			if (trim($this->piVars['DATA']['forgot_email']) && t3lib_div::validEmail($this->piVars['DATA']['forgot_email']))	{
				...
				//get firstname as well - Jaspreet Singh
				$query="SELECT username,password,first_name FROM fe_users WHERE email='".addslashes(trim($this->piVars['DATA']['forgot_email']))."' 
				...
				if ($row=mysql_fetch_assoc($res))	{
					//Use the firstname for the greeting - Jaspreet Singh
					//OK, remove the firstname.
					$msg=sprintf($this->pi_getLL('forgot_password_pswmsg','',1),'',$row['username'],$row['password']);
				} else {
...
//print_r($this->conf); //jsdebug
			...
			if ($GLOBALS['TSFE']->loginUser)	{
				if ($logintype=='login')	{
					...
					$content.='<script language="javascript" type="text/javascript">opener.location.reload();</script>';
	
					if ($outH)	$content.='<h3>'.$outH.'</h3>';
					if ($outC)	$content.='<p>'.$outC.'</p>';
					
					$content.='<p align="center"><a href="#" onClick="self.close();return false;">Close this window</a></p>';
					...
					$outC = str_replace('###USER_FIRST_NAME###',$GLOBALS['TSFE']->fe_user->user['first_name'],$outC);
					
					if ($outH)	$content.='<h3>'.$outH.'</h3>';
					if ($outC)	$content.='<p>'.$outC.'</p>';
					
					$usernameInfo = '<strong>'.htmlspecialchars($GLOBALS['TSFE']->fe_user->user['username']).'</strong>';
					...
					if (!$this->conf['welcomescreen']){
						$topinfo = '						<table '.$this->conf['tableParams_details'].'>
							<tr>
								<td><p'.$this->pi_classParam('username').'>Edit
								Account</p>
								<p>'.$usernameInfo.'</p></td>
							</tr>
							<tr>
						...
						$submit_value= '<input type="submit" name="submit"   id="'.$this->pi_getClassName('logout_submit').'" value="'.$this->pi_getLL('logout','',1).'"'.$this->pi_classParam('submit').' />';
					}else{
						$topinfo= $bottominfo= '';
						$submit_value= '<li id="logout"><a href="#" onclick="logout_form.submit(); return false;">Logout</a></li>';
					}
					$content.='
						...
						<!--
							"Logout" form
						-->
						<form action="'.htmlspecialchars($this->pi_getPageLink($GLOBALS['TSFE']->id,'_top')).'" target="_top" method="post" id="logout_form" name="logout_form">'.$topinfo.'
'.$submit_value.'
									<input type="hidden" name="logintype" value="logout" />
									<input type="hidden" name="pid" value="'.intval($d['_STORAGE_PID']).'" />'.$bottominfo.'
						</form>
					';
				}
			} else {
				...
				$content.='<form action="'.htmlspecialchars($this->pi_getPageLink($GLOBALS['TSFE']->id,'_top')).'" target="_top" method="post" '.(strlen($onSubmit)?' onSubmit="'.$onSubmit.'"':'').'>
		<label>'.$this->pi_getLL('username','',1).'</label>
		<input size="17" type="text" name="user" value="" id="'.$this->pi_getClassName('user').'" />
		<label>'.$this->pi_getLL('password','',1).'</label>
		<input size="17" type="password" name="pass" value="" id="'.$this->pi_getClassName('pass').'" />
		<input type="image" src="/fileadmin/templates/includes/images/form-enter.jpg" name="submit" value="'.$this->pi_getLL('login','',1).'"'.$this->pi_classParam('submit').'   id="'.$this->pi_getClassName('login_submit').'" />
